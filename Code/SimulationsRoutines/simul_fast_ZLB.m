function [gap_lag_simu, pi_simu , gap_simu,b_pi_simu,b_gap_simu,...
    ...
    welfare_pi_inst ] = simul_fast_ZLB(parpolicy, fspace, bench_p, model, ...
        gaintype, shocktype, sim_parameters)

% SIMUL_FAST_ZLB simulates series or IRFs for the case in which the ZLB is taken into account. This is not a full blown ZLB solution, just imposing the ZLB on the simulation while the solution is unconstrained by ZLB.
%
% INPUTS:
...
%     gap_zero: initial condition for output gap
%     b_gap_zero: initial condition for learning coefficient for output gap
%     b_pi_zero: initial condition for learning coefficient for inflation
%     gamma_t_zero: initial condition for the learning parameter
%     sim_parameters: parameters for the simulation
%     Basis: basis functions calculated in the grid
%
% OUTPUT: SERIES FOR
%
...
for jjj = 1:draws
    ...
    %% Set initial conditions
     [b_pi_simu(:,1), b_gap_simu(:,1), gamma_t_simu(:,1), gap_lag_simu(:,1)] = ...
        set_initial_conditions(model, b_pi_comm, b_x_comm, ...
                b_pi_zero, b_gap_zero, gamma_t_zero, gap_zero,gaintype, bench_p);
    ...
    for i=1: periods_simulations
        ...
        [gap_simu(:,i), pi_simu(:,i), gap_lag_simu(:,i+1), ...
            ...
            allocations_in_period_i_fast_ZLB(model, gaintype, states, fspace, parpolicy,...
