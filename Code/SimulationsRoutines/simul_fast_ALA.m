function [gap_lag_simu, pi_simu , gap_simu,b_pi_simu,b_gap_simu,...
    ...
    welfare_pi_inst, R_simu ] = simul_fast_ALA(parpolicy, fspace, bench_p, model, ...
        gaintype, shocktype, sim_parameters, OLS_flag, ALA)

% SIMUL_FAST_ALA simulates series or IRFs for the MMS paper (ALTERNATIVE LEARNING ALGORITHMS).
%
% INPUTS:
...
%     OLS_flag: if 'yes', it means we are using OLS learning as ALA
%     ALA: could be 'OLS', '001', '008', 'bench', 'decr', describes which ALA we are looking at
%
% OUTPUT: SERIES FOR
%
...
%     R_simu: the series for R_t (estimate of the variance) 
...
[alpha, betta, kappa, sig, gam, nphi, sigma, rho, ...
    ...
    sigeps, thmax, thmin, LowerBound, UpperBound, imp] = translate_parameters(bench_p);
...
% create R_zero in case we are using OLS learning
if strcmp(OLS_flag,'yes')
    R_zero = (sigeps.^2)./(1-b_gap_zero.^2);
else
    R_zero = 1;
end
...
for jjj = 1:draws
    ...
    %% Set initial conditions
     [b_pi_simu(:,1), b_gap_simu(:,1), gamma_t_simu(:,1), gap_lag_simu(:,1), R_simu(:,1)] = ...
    ...
    for i=1: periods_simulations
        % set state variables 
        [states, gamma_t_simu(:,i)] = states_in_period_i_ALA(gap_lag_simu(:,i),...
            b_pi_simu(:,i),gamma_t_simu(:,i),gam,costpushshock(:,i),...
            gaintype,shutdownlearning, ALA);
        
        % Put gamma_t to decreasing gain when ALA = 'decr' or 'OLS'
        if strcmp(ALA, 'decr') %|| strcmp(ALA, 'OLS')
            gamma_t_simu(:,i) = ones(intdraws,1)./i;
        end
        
        % set R_simu
        if strcmp(ALA, 'OLS')
            R_simu(:,i+1) = R_simu(:,i) + ...
                gamma_t_simu(:,i).*(gap_lag_simu(:,i).^2 - R_simu(:,i) );
        else
            R_simu(:,i+1) = 1;
        end
        ...
        [gap_simu(:,i), pi_simu(:,i), gap_lag_simu(:,i+1), ...
            ...
            bench_p, b_gap_simu(:,i),sim_parameters, R_simu(:,i+1), ALA, gamma_t_simu(:,i));
