...
[alpha, betta, kappa, sig, gam, ~, sigma, ~, ...
    ~, ~, ~, ~, b_x_comm, ...
...
switch model
    case 'MMS'
        ...
        intrate_simu = - sig.*gap_simu.*( 1 - b_gap_simu - b_pi_simu./sig );
        gap_simu(intrate_simu<=0) = 0; % reset gap_simu to zero when the ZLB binds
        intrate_simu(intrate_simu<=0) = 0; 
