% File to generate Figure 7b
% Consumption equivalent on a rolling window (as in Figure 1)
% Sensitivity analysis: we vary the discount factor beta to check for consistency
...
% Set the vector containing the betas we want to test
beta_list = [.96;  .999 ];

%% LOOP OVER beta's
for index_beta = 1:length(beta_list)
    
    bench_p.betta = beta_list(index_beta);
    ...
    %% Figure 1: build it recursively
    if index_beta == 1
        figure(1);
    end
    
    plot(1:length(mcce_MMS_from_PLT),mcce_MMS_from_PLT,...
        'DisplayName',['Opt, \beta = ' num2str(beta_list(index_beta))], 'LineWidth',2);
    hold on;
    
    plot(1:length(mcce_EHCOMM_from_PLT), mcce_EHCOMM_from_PLT,...
        'DisplayName',['EH PLT, \beta = ' num2str(beta_list(index_beta))], 'LineWidth',2);
...
print -f1 -r600 -depsc2 figure7b;
saveas(1,'figure7b','fig');
